...
function draftPayload(fields, saveMode, rowVersion) {
  const payload = {
    save_mode: saveMode,
    ...
    special_handling_at: fields.handledAt.value ? `${fields.handledAt.value}+08:00` : "",
...
export function createRecordDrawer(documentRef, options) {
  ...
  handler.value = current.handler_user_id || user?.id || "";
  ...
  const fields = {
    ...
    handledAt: element(documentRef, "input", { type: "datetime-local", step: "1", value: isoLocalValue(current.special_handling_at || options.now), "aria-label": "特殊处理时间", disabled: !canEdit }),
  ...
  function showError(error) {
    ...
    Object.entries(error.fields || {}).forEach(([fieldName, message]) => {
      const control = fields[fieldName] || fields[fieldName === "special_handling_at" ? "handledAt" : fieldName];
      if (control) {
  ...
  const saveDraft = () => run(
    () => creating
      ? actions.createRecord(draftPayload(fields, "draft", null))
      : actions.updateRecord(current.id, draftPayload(fields, "draft", current.row_version)),
    "草稿已保存",
  );
  const saveRecord = () => run(
    () => creating
      ? actions.createRecord(draftPayload(fields, "record", null))
      : actions.updateRecord(current.id, draftPayload(fields, "record", current.row_version)),
  ...
  async function loadAudit(page = 1) {
    ...
    try {
      ...
      (data.items || []).forEach((item) => auditBody.append(element(documentRef, "tr", {}, [
        element(documentRef, "td", { text: item.occurred_at }),
  ...
  const header = element(documentRef, "header", { className: "rsp-drawer-head" }, [
    element(documentRef, "div", {}, [
      element(documentRef, "span", { className: "rsp-eyebrow", text: creating ? "数据录入" : current.record_no || "记录详情" }),
      element(documentRef, "h2", { text: creating ? "新建报表特殊处理" : current.summary || "报表特殊处理详情" }),
    ]),
    actionButton(documentRef, "收起详情", "rsp-button-secondary", onClose),
  ]);
  const basic = element(documentRef, "section", { className: "rsp-drawer-section" }, [
    element(documentRef, "h3", { text: "基本信息" }),
    element(documentRef, "div", { className: "rsp-form-grid" }, [
      ...
      labeledField(documentRef, "特殊处理时间", fields.handledAt),
      labeledField(documentRef, "处理人", fields.handler),
      element(documentRef, "div", { className: "rsp-workflow-state" }, [
        element(documentRef, "span", { text: "流程状态" }),
        element(documentRef, "strong", { text: "未启用" }),
        element(documentRef, "small", { text: "审批流程将在后续阶段启用" }),
      ]),
      canAdmin ? labeledField(documentRef, "操作原因", fields.reason, "rsp-span-two") : null,
    ]),
  ]);
  const content = element(documentRef, "section", { className: "rsp-drawer-section" }, [
  ...
  const script = element(documentRef, "section", { className: "rsp-drawer-section" }, [
  ...
  const audit = element(documentRef, "section", { className: "rsp-drawer-section rsp-audit", hidden: creating ? "" : null }, [
  ...
  const footer = element(documentRef, "footer", { className: "rsp-drawer-actions" }, footerButtons);
  const drawer = element(documentRef, "aside", {
    className: "rsp-record-drawer",
    role: "dialog",
    "aria-modal": "false",
    "aria-label": creating ? "新建报表特殊处理" : "报表特殊处理详情",
  }, [header, errorBox, basic, content, script, audit, footer]);
  loadAudit(1);
  return drawer;
}

...
- 权限：按角色能力矩阵 `rsp.view/create/edit/confirm/void/reopen/delete` 判定；标准用户遵循“谁创建谁处理”（仅创建人可编辑/作废/重开，处理人不可单独操作）；`rsp.confirm` 控制完成确认；`rsp.delete` 控制任意状态物理删除。管理员默认拥有全部能力。
- 状态：`draft → pending → processing/completed`，`processing → pending/completed`；具备作废能力可将 `draft/pending/processing` 作废；具备重开能力可将 `completed/voided` 重开为 `pending`；具备删除能力可对任意状态记录物理删除（二次确认，不可恢复）。

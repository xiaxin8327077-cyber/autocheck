...
BUILTIN_REGION_SEEDS = (
    ...
    RegionSeed("report_submission", "quarterly_special_processing", "季度报表特殊处理", "list", True, "system", 60, "按报送期月度统计待确认和已确认的特殊处理数量，并按季度汇总展示。"),
...
BUILTIN_FIELD_SEEDS = (
    ...
    FieldSeed("monthly_trust_projects", "month", "月份", "string", False, 10, "接口展示格式：1月～12月，例如：8月。来源 SQL 推荐返回 YYYY-MM，例如：2026-08，用于明确年份；兼容仅月份时须由 SQL 限定业务年份。"),
    ...
    FieldSeed("quarterly_special_processing", "quarter", "季度", "string", False, 10, "对外接口按季度返回。内部来源按报送期月份统计后汇总。"),

...
function CompareModal({ file = CHANGED_FILES[0], onClose }) {
  return <div className="modal-mask" role="dialog" aria-modal="true"><div className="modal diff-modal">
    <div className="modal-head"><div><h3>查看修改 · {file.path.split('/').pop()}</h3><p>对比开发工作区当前文件与基准版本 r1842</p></div><button className="text-btn" onClick={onClose}>关闭</button></div>
    ...
    <div className="diff-columns">
      <section><header><b>修改前</b><span>基准版本 r1842</span></header><dl><div><dt>步骤名称</dt><dd>获取公司信息</dd></div><div><dt>数据库连接</dt><dd>ass_man_reg_24</dd></div><div className="diff-line old"><dt>查询条件</dt><dd>report_date = ${'{caldate_t1}'}</dd></div><div><dt>失败处理</dt><dd>停止后续步骤</dd></div><div><dt>输出字段</dt><dd>company_id, company_name, id_no</dd></div></dl></section>
...
function PublishModal({ revision, restore = false, onClose, onConfirm }) {
  return <div className="modal-mask" role="dialog" aria-modal="true"><div className="modal publish-modal">
    <div className="modal-head"><div><h3>{restore ? '恢复历史版本' : '发布到生产'}</h3><p>{restore ? '把历史版本重新发布到生产目录' : '将已提交的版本发布到生产目录'}</p></div><button className="text-btn" onClick={onClose}>关闭</button></div>
    <div className="release-route"><div><span>版本来源</span><strong>版本 {revision}</strong><small>已提交、内容不可变</small></div><b>→</b><div><span>发布目标</span><strong>/home/kettle/prod</strong><small>生产目录，只允许平台发布</small></div></div>
...
function KettleWorkspace({ notify, onGoVersions }) {
  ...
  return <>
    ...
    <section className={`webspoon-shell kettle-canvas ${expanded ? 'expanded' : ''}`}>
      ...
      <div className="webspoon-frame-wrap"><iframe key={frameKey} title="WebSpoon Kettle 在线设计器" src="/spoon/spoon" /><div className="frame-fallback"><strong>正在加载 WebSpoon</strong><span>若服务器禁止嵌入，正式接入时需调整 WebSpoon 的 Frame/CSP 配置。</span></div></div>
    </section>
...
function VersionManagement({ notify }) {
  ...
  return <>
    ...
    {commitOpen && <CommitModal onClose={() => setCommitOpen(false)} onConfirm={() => { setCommitOpen(false); notify('已提交版本，新版本为 r1843'); }} />}
...
export function SchedulingManagement({ section, notify, onNavigate }) {
  const subtitles = { KETTLE: '在线编辑开发目录中的 KJB/KTR 文件', 版本管理: '管理版本提交、生产发布与历史恢复', 定时调度: '统一配置 Kettle 与 Spider 的生产定时任务' };

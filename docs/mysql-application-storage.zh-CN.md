...
当前版本使用 MySQL 应用库 `auto_check` 保存系统自身配置、用户、认证数据、执行历史、报送导航配置/快照、界面偏好和模块宿主状态。DWS 数据源和报表数据源仍由页面配置维护，继续支持 PostgreSQL 或 MySQL，不属于本应用库。模块系统升级后完整应用结构为 42 张表，`app_schema_version` 仍为 `1`。
...
16. 配置 `config.json` 的 `app_database` 节点。
17. 启动应用并确认连接、42 张表、全局结构版本和关键表数据。

建表/升级脚本不包含 `CREATE DATABASE`、`DROP`、`TRUNCATE`、生产凭据或业务数据。`004`、`006`、`008` 和 `012_module_system.sql` 使用 `CREATE TABLE IF NOT EXISTS`；`005` 与 `007` 通过 `information_schema` 判断字段或约束是否存在后再升级。升级脚本可按顺序重复执行；`008` 会幂等写入年度法定节假日和调休工作日配置，`009` 规范历史人工确认范围，`010` 将第七步改为仅展示并补齐第六步归档时间字段映射，`011` 将归档类完成时间统一改为仅取 `create_date`，`012` 只创建模块平台表且不修改 `app_schema_version`。如果目标表已经存在，仍需人工核对字段和约束是否符合当前规范。
...
- 当前完整 42 张应用存储表结构齐全，已按顺序应用至 `011_report_navigation_completion_time_sources.sql`，并在备份后由运维人工执行 `012_module_system.sql`；模块业务表不属于全局 `EXPECTED_APP_SCHEMA`。

...
当前版本使用 MySQL 应用库 `auto_check` 保存系统自身配置、用户、认证数据和执行历史。DWS 数据源和报表数据源仍由页面配置维护，继续支持 PostgreSQL 或 MySQL，不属于本应用库。
...
5. 配置 `config.json` 的 `app_database` 节点。
6. 启动应用并确认连接、结构版本和关键表数据。

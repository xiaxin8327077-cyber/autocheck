...
- 2026 年“月度信托项目数量”初始化 1—7 月（7 月单一信托 3、集合信托 49、财产权信托 36），“报表对账完成时间”初始化 1—6 月，“报表校验问题处理”初始化 1—9 月（9 月为 18 条）；“季度报表特殊处理”内部初始化 1—6 月固定估分 `9、12、10、11、10、13` 和 7 月 `8`，8 月及以后按 `report_period` 实时统计。
...
- [x] **Step 1: 写迁移、schema 和基线失败测试**
  ...
  断言 `002_year_snapshots.sql` 只创建 `dashboard_management_year_snapshots`、模块注册新表全部字段，并在 `seed_builtin_catalog()` 后得到 2026 年信托 1—7 月和对账完成时间 1—6 月、校验问题处理 1—9 月（9 月 18 条）和特殊处理 1—7 月数据；重复初始化不覆盖已采集的同月值，也不把基线写入 2027 年。月度调整不新增迁移，现行 schema 保持 5。
...
- Spec coverage：四个快照区域、实时成功覆盖、缺期保留、失败回退、当年过滤、2026 已确认基线（校验问题处理到 9 月且 9 月为 18 条，信托到 7 月、对账时间到 6 月、特殊处理固定到 7 月）、无后台任务、回滚均有对应任务。

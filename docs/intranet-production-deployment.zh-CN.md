...
15a. 继续按编号执行 `014_role_capability_settings.sql` 至 `023_report_navigation_schedule_prepare.sql`；`022` 新增统一定时任务表并预置报送导航统计、通知过期清理和逐笔校验字段映射刷新。
...
18. 上线验收需分别确认原 20 张迁移目标表的数据行数与迁移报告一致，以及当前完整 56 张应用存储表结构与配置升级（在备份下人工执行 `012_module_system.sql`、`013_report_navigation_provider_states.sql` 及后续脚本至 `023_report_navigation_schedule_prepare.sql`）齐全；删除旧 SQLite `auto-check.db` 后应用仍应只依赖 MySQL 应用库运行。

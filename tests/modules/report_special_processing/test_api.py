...
def test_api_registers_contract_routes_without_delete_and_enforces_body_limit():
    router = _router()
    ...
    assert _dispatch(router, "DELETE", "/records/1", user={"role": "admin"}) is not None
    assert _dispatch(router, "DELETE", "/records/1", user={"role": "admin"}).status in {404, 405}


def test_api_permissions_keep_admin_operations_admin_only():
    router = _router()
    assert _dispatch(router, "POST", "/records/1/void", body={}, user={"role": "user"}).status == 403
...
def test_api_maps_domain_and_unknown_errors_without_leaking_details():
    ...
    class Broken(Service):
        def catalog(self): raise RuntimeError("mysql://secret script DROP TABLE path C:/secret")

...
def test_expected_schema_is_immutable_and_contains_expected_tables():
    assert len(EXPECTED_APP_SCHEMA) == 55
    assert EXPECTED_APP_SCHEMA["system_dictionaries"] >= frozenset(
        {"dictionary_code", "dictionary_name", "enabled", "system_locked", "sort_order"}
    )
    assert EXPECTED_APP_SCHEMA["system_dictionary_items"] >= frozenset(
        {"id", "dictionary_code", "item_code", "item_name", "enabled", "sort_order"}
    )
